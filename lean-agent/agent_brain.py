...
def build_system_prompt(workspace: str, agent_mode: str = "zerobound") -> str:
    ...
    if agent_mode == "marimo":
        return common_prefix + (
            ...
            "You are running in an orchestrated agent loop. You MUST begin EVERY response with exactly one of the following tags on the very first line:\n"
            "1. `MODE: TOOL_CALLS` — Use this if you need to execute one or more tools. After this tag, output your CALL blocks. The user will NOT see this response, only the system, which will execute the tools and reply with the results.\n"
            "2. `MODE: REPORT` — Use this when you have finished your work and are ready to communicate the final answer or code directly to the user. After this tag, write your final markdown response.\n"
            "If you do not prefix your response with one of these tags, the orchestration loop will fail!\n"
    ...
    async def run(self, user_input: str, callback: Optional[Callable] = None, images: Optional[List[str]] = None):
        ...
        while True:
            ...
            if loop_count > self.config.max_iterations:
                ...
                return err
            system_prompt = build_system_prompt(CURRENT_WORKSPACE, agent_mode=self.config.agent_mode)
